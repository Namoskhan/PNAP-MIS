# Basic Unit Current Pipeline Mapping (Phase O8)

## 1. Context and Function Entry Point
- **File**: `pnap-mis/server/src/services/analyticsService.js`
- **Method**: `officeBearerActivity('BASIC_UNIT')`
- **Caller**: `orgSnapshot(f)` via `Promise.all([ officeBearerActivity('BASIC_UNIT'), ... ])`
...
1. **Outer `$match`**:
   ...
   - **Matching volume**:
     - 10k: 3,000 documents (across 1,000 units).
     - 50k: 15,000 documents (across 5,000 units).
2. **Member `$lookup`**:
   ...
   - **Execution count**:
     - 10k: 3,000 executions of the lookup subquery.
     - 50k: 15,000 executions of the lookup subquery.
3. **`$unwind`**:
   - `path: '$member'`, `preserveNullAndEmptyArrays: false`.
   - Flattens the 1-element member array produced by `$lookup`.
4. **`$group`**:
   - `_id: '$unitId'`: Groups role assignment records by basic unit ID.
   - Accumulator `lastActivityAt: { $max: '$member.lastActivityAt' }`: Tracks the most recent activity timestamp across all office bearers of that unit.
...
```
- **Officer Priority Order**: `['PRESIDENT', 'CHAIRMAN', 'GENERAL_SECRETARY', 'SECRETARY', 'SENIOR_MAWIN', 'FINANCE_SECRETARY']`. For Basic Unit, `SECRETARY` is chosen first, fallback to `SENIOR_MAWIN`, fallback to `FINANCE_SECRETARY`, fallback to `officers[0]`.
- **Unit Output Shape**:
